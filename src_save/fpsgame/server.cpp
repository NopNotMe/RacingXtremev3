    ...
    bool setmaster(clientinfo *ci, bool val, const char *pass = "", const char *authname = NULL, const char *authdesc = NULL, int authpriv = PRIV_MASTER, bool force = false, bool trial = false)
    {
        ...
        if(val)
        {
            ...
            else if(wantpriv <= PRIV_MASTER && !force)
            {
                ...
                loopv(clients) if(ci!=clients[i] && clients[i]->privilege && authname != NULL) //add authname xtreme check for racemod
                {
    ...
    void parsepacket(int sender, int chan, packetbuf &p)     // has to parse exactly each byte of the packet
    {
        ...
        while((curmsg = p.length()) < p.maxlen) switch(type = checktype(getint(p), ci))
        {
            ...
            case N_MAPCRC:
            {
                ...
                remod::onevent(ONCRC, "ii", ci->clientnum, ((uint)ci->mapcrc == mcrc));
                break;
            }
            ...
            case N_SHOOT:
            {
                ...
                if(cq) 
                {
                    ...
                    cq->setpushed();
                    if (shot->gun == GUN_GL)
                      cq->state.throwingnade = true; // racing - nade exploded on cq
                }
                else delete shot;
                break;
            }
