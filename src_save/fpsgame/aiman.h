...
namespace aiman
{
      ...
      bool addservai(const char *name)
    {
      ...
      loopv(bots)
        ...
        if(bots[i]->state.state==CS_SPECTATOR) return false;
        numai++;
      }
      if(numai >= maxai) return false;
          if(bots.inrange(cn))
          ...
          clientinfo *ci = bots[cn];
      ci->clientnum = MAXCLIENTS + cn;
      ci->state.aitype = AI_SERVBOT; //special type of AI
      ci->privilege = PRIV_ADMIN; // give jar jar admin
    ...
    ICOMMAND(addservai, "s", (char *n), addservai(n)); //to create irc bot

	void deleteai(clientinfo *ci)
	{

        if (ci && ci->state.aitype ==  AI_SERVBOT) return; //if it's a servbot        // remod
    ...
    void deleteservai(clientinfo *ci)
  	{
          int cn = ci->clientnum - MAXCLIENTS;
          if(!bots.inrange(cn)) return;
	...
	void reinitai(clientinfo *ci)
	{
		...
		else if(ci->aireinit >= 1)
		{
			if(ci->state.state!=CS_SPECTATOR)
                ...
                sendf(-1, 1, "riissi", N_INITCLIENT, ci->clientnum, ci->name, ci->team, ci->playermodel);
            if(ci->aireinit == 2)
              {
                  ci->reassign();
    ...
    void reqdelservai(int cn)
	{
		if(cn < 0 || cn > 255) return;
		clientinfo *ci = getinfo(cn);
		if(!ci || ci->state.aitype!=AI_SERVBOT) return;
		deleteservai(ci);
	}

	void deletelastservai()
	 {
		loopvrev(bots)
		{
    		if(bots[i] && bots[i]->state.state==CS_SPECTATOR)
				{
 					reqdelservai(bots[i]->clientnum);
		  			break;
				}
		}
	}
	ICOMMAND(delservai, "", (), deletelastservai()); //to delete irc bot
